...
2. Click on the **Browse** tab, and among all the profiles that were already created for you or that you created as part of previous labs, you should see these three profiles. 

Find the corresponding Experience Events for each profile in the Postman collection 

1. If necessary, open Postman
2. Ensure that the **EDGE\_REGION** and **DATASTREAM\_CONFIG** environment variables are still set. If they need to be set again, review the steps in the 'Import Environment and Collection' lab.
3. Expand the **Decisioning Lab** folder. You will see 2 Experience Events for each profile:
