...
>[!NOTE]
>
>Edge profile access is specifically designed for high throughput, low latency use cases such as web/mobile inbound personalization and real-time offer decisioning. For lower throughput scenarios such as agent assisted support or sales interactions, the Hub profile lookup API is more appropriate. See the [Real-time Profile Access for Support and Sales Scenarios blueprint](/help/blueprints/use-case-patterns/audience-building-activation/real-time-profile-lookup.md) for hub-based profile access.
