---
title: Lab exercise
description: Work through a printable workbook mapping three Connection 5G use cases to the right AJO architecture components and orchestration engine.
doc-type: article

solution: Experience Platform
exl-id: 9e8f24a4-f206-41fe-8107-7184c0761698
---

# Lab exercise

**You'll need the following before you begin:**

- Mapping Use Cases to Architecture Workbook — [Print Workbook Below]
- Pen, pencil, or highlighter

Download File — [Mapping Capabilities to Architecture Exercise.pdf](assets/lab-exercise-mapping-capabilities-to-architecture.pdf)

**For each of the three Connection 5G use cases, you will:**

- Read the use case brief and appendix materials
- Answer the five critical thinking questions
- Circle or highlight the AJO architecture components needed
- Make your final orchestration engine recommendation

Work through all three use cases, then continue to the review video when you're done.
