...
1. Paste the URL of the `Environment File` into the import modal text box on the overlay.  This action triggers an automatic import
...
Once imported, validate your environment file exists by clicking on the `Environments` tab in the left sidebar.  You see something similar to below.
...
Before making any API calls, you need to update a few of the variables in the environment file you just imported.  These variables are referenced in the API calls so be sure they are correctly filled in.  The variables are broken into two groups:
...
1. Update the `current values` for the variables listed below:
   - CLIENT\_SECRET
   - CLIENT\_ID (also called API KEY)
   - TECHNICAL\_ACCOUNT\_ID
   - IMS\_ORG

When done your environment file should look similar to this image:
...
>[!NOTE]
>
>If you're working through these labs independently rather than at a live training event with a sandbox-assignment.pdf, find both values while logged into your sandbox from the Adobe Experience Platform UI URL. For example:
>
